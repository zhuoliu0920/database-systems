...
public class rq {
	...
	public static void main(String[] args) {	
		...
		boolean case2 = false;
		boolean case1 = false;
		String N,colJoined1,colJoinedList1;
		N=colJoined1=colJoinedList1="";
		try {
		...
		try {
			...
			con.connect();
			/*********/	
			String inputFileName = getName("filename", ",", args[0]);
			String inputSql = readFile(inputFileName);
	...
	"(\\w.*\\w)\\s+FROM\\s+R\\s+JOIN.*ON.*WHERE\\s+(\\w+)\\s*\\<\\s*(\\d+)\\s*\\)\\s*;\\s*(SELECT.*);", Pattern.CASE_INSENSITIVE
            ...
            boolean boolQueryDistinct = finalQuery.matches(".*DISTINCT.*");
			boolean boolQueryJoin2 = finalQuery.matches(".*JOIN.*AS.*JOIN.*AS.*");
			boolean boolQueryJoin1 =  boolQueryJoin2 ? false : finalQuery.matches(".*JOIN.*");
			Pattern patternforO1 = Pattern.compile(
			"SELECT\\s+DISTINCT\\s+(\\w+)\\s*((\\,\\s*\\w+\\s*)+)?(INTO\\s+(\\w+))?\\s+FROM\\s+(\\w+)\\s*", 
			        Pattern.CASE_INSENSITIVE);
			Matcher matcherforO1 = patternforO1.matcher(finalQuery);
			Pattern patternforJoin = null;
			if (boolQueryJoin2) {
 
System.out.println("11");
			    patternforJoin = Pattern.compile(
			   "SELECT\\s+(\\w.*)\\s+INTO\\s+(\\w+)\\s+FROM\\s+(\\w+)\\s+"+ 
			      "(JOIN\\s+(\\w+)\\s+AS\\s+(\\w+)\\s+ON\\s+(\\w+)\\.(\\w+)\\s*=\\s*(\\w+)\\.(\\w+)\\s+)" +
				  "(JOIN\\s+(\\w+)\\s+AS\\s+(\\w+)\\s+ON\\s+(\\w+)\\.(\\w+)\\s*=\\s*(\\w+)\\.(\\w+)\\s*)"
			       , Pattern.CASE_INSENSITIVE);
		    }
			else if (boolQueryJoin1) { 
System.out.println("111");
			     patternforJoin = Pattern.compile(
			   "SELECT\\s+(?:(?:(?:R\\.\\w+)|(?:\\w+))\\s*\\,\\s)+\\s*([^R]\\..*)+\\s+INTO\\s+(\\w+)\\s+FROM\\s+(\\w+)\\s+"+ 
			      "(JOIN\\s+(\\w+)\\s+(ON)\\s+(\\w+)\\.(\\w+)\\s*=\\s*(\\w+)\\.(\\w+)\\s+)"
			       , Pattern.CASE_INSENSITIVE);
			}
			else {
			     patternforJoin = Pattern.compile("");
...
System.out.println(finalQuery);
           Matcher matcherforO2 = patternforJoin.matcher(finalQuery);
System.out.println("2");
System.out.println(finalQuery);
//matcherforO1 =matcherforJoin; //WCA

/* OPTIMIZATION OF SELECT DISTINCT  */
        if (boolQueryDistinct) {
			...
			String columnO1 = "";
			finalTableName = matcherforO1.group(matcherforO1.groupCount()-1);
			isINTO = (matcherforO1.group(matcherforO1.groupCount()-2)==null)?false:true;
			...
			vExist = (columnO1.indexOf(columnV) >= 0)?true:false;
			pExist = (columnO1.indexOf(matcher.group(4)) >= 0)?true:false;
			dExist = (columnO1.indexOf(matcher.group(1)) >= 0)?true:false;
...
/* END OPTIMIZATION SELECT DISTINCT */

/* OPTIMIZATION OF JOIN  */
        if (boolQueryJoin1) {
		    System.out.println("IN JOIN 1");
			matcherforO2.reset();
			matcherforO2.find();
			for (int g=1; g<=matcherforO2.groupCount(); g++) {
			    System.out.println( "group:" + g+  "="+matcherforO2.group(g));
			}
			N = matcherforO2.group(5)  ;
			colJoined1=  matcherforO2.group(10)  ;
			colJoinedList1=  matcherforO2.group(1)   ;
			String tableO1 = matcherforO2.group(2)   ;
			String columnO1 = "";
			finalTableName = matcherforO2.group(2);
			finalQuery.matches(".*DISTINCT.*");
			isINTO = finalQuery.matches(".*INTO.*FROM.*");
			// if (matcherforO2.group(2) == null) {
				// columnO1 = matcherforO2.group(1);
			// }
			// else {
				// columnO1 = matcherforO2.group(1) + matcherforO2.group(2);
			// }
			// System.out.println(columnO1);
		...
		/* END JOIN  */
			...
			else if (O2 && boolQueryJoin1) {
				System.out.println(">>>>>");
				// Create table T and initialize result table R
				initializeTableO2_1(con, inTable, columnI, columnJ, columnV,
                                 N,  colJoined1,  colJoinedList1,queryRecorder);
 
				// Use semi-naive algorithm to update result table R
				int d = 1; 
				while (d == checkResultTable(con, queryRecorder) && d < maxDepth) {
					System.out.println("Updating result table R in depth " + (d+1) + "...");
					updateResultTableO2_1(con, inTable, columnI, columnJ, columnV,  colJoinedList1.replace(N+".", "r1."),queryRecorder);
					d++;
				}
				if  (d == maxDepth)
					System.out.println("Reaching maximum recursion depth, stop updating...");
	...
	private static void updateResultTableO2_1(DatabaseConnector con, String E,String i, String j, String v, String colJoinedList1,PrintWriter queryRecorder) {
		...
		try {
			// Insert paths with depth d+1 into result table
            		...
            		SQL =	"INSERT INTO R\n" +
				"SELECT r1." + i + " as " + i + ", t1." + j + " as " + j + ", r1." + v + "+t1." + v + " as " + v + 
				   ", r1.p*t1.p as p, r1.d+1 as d, " + colJoinedList1 + "\n" +
				"FROM (\n" +
				"(SELECT *\n" +
				"FROM R\n" +
				"WHERE d = (SELECT MAX(d) FROM R) AND " + i + "<>" + j + ") AS r1\n" +
	...
	private static void initializeTableO2_1(DatabaseConnector con, String E, String i, String j, 
	                                      String v, String N, String colJoined1, String colJoinedList1, PrintWriter queryRecorder) {
		...
		String SQL = "";
			...
			SQL =	"SELECT " + E+"."+ i + "," + E+"."+j + "," +v + ","  + colJoinedList1 + " INTO TN " +
				" FROM " + E + " JOIN " + N + " ON " + E+"." + i +"=" + N+"." + colJoined1 +"\n;";
			queryRecorder.println(SQL);
            		con.execute(SQL);
					
			colJoinedList1 = colJoinedList1.replace(N+".", "TN.");		
			SQL = 	//"INSERT INTO R\n" +
				"SELECT " + i + "," + j + "," +	v + ",1 AS p,1 AS d," +  colJoinedList1 +"  INTO  R\n" +
				"FROM TN "+ ";\n";
			queryRecorder.println(SQL);
            		con.execute(SQL);
